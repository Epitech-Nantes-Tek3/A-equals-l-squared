...
// about.json route answering the client IP, the current time and the services
// data
app.get('/about.json', (req, res) => {
  try {
    const about = {};
    about.client = {host : req.ip} about.server = {
...
app.get('/database/get/try', async (req, res) => {
  try {
    const allUsers =
        await prisma.try.findMany(); /// GET all the user and her relations
    res.send(allUsers);
  } catch (err) {
    console.log(err);
    res.status(500).send(err);
...
app.get('/database/post/user',
        async (req, res) => { /// ?name=alice&posts=hey&profile=bio
          try {
...
app.get('/database/update/user/name',
        async (req, res) => { /// ?id=1&name=pierre
          try {
