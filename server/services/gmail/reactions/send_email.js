...
async function gmailSendEmailFromArea(Area) {
  try {
    ...
    const from =
        reactionParameters.find(parameter => parameter.Parameter.name == 'from')
            .value
    const to =
        reactionParameters.find(parameter => parameter.Parameter.name == 'to')
            .value
    const subject =
        reactionParameters
            .find(parameter => parameter.Parameter.name == 'subject')
            .value
    const body =
        reactionParameters.find(parameter => parameter.Parameter.name == 'body')
            .value
...
function getBase64message(from, to, subject, body) {
  const message = [
    'From: ' + from, 'To: ' + to, 'Content-Type: text/html; charset=utf-8',
    'MIME-Version: 1.0', 'Subject: ' + subject, '', body
  ].join('\n');
 ...
 */
const sendEmail =
    async (from = null, to, subject, body) => {
  if (from == null || from === 'aequallsquared@gmail.com') {
    ...
    return sendEmailWithAccessToken(from, to,
                                    subject, body);
  }
}
