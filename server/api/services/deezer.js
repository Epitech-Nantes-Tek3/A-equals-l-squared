...
module.exports = function(app, passport, database) {
  ...
  app.get(
      '/api/services/deezer/getUserPlaylists',
      passport.authenticate('jwt', {sessions: false}), async (req, res) => {
        if (req.user.deezerToken == null)
          return res.status(400).send('No Deezer account linked.')
          try {
            const playlists =
                await getUserPlaylists(req.user.deezerId, req.user.deezerToken)
            if (playlists == null) return res.status(400).send(
                'No playlist found.')
