...
module.exports = function(app, passport, database) {
    ...
    try {
      ...
      if (oldUser) {
        user = await database.prisma.user.update(
            {where: {email: req.body.email}, data: {googleId: req.body.id}})
        const token = utils.generateToken(user.id)
        return res.status(201).json({
          status: 'success',
          data: {user, token},
      ...
      const token = utils.generateToken(user.id)
      return res.status(201).json(
          {status: 'success', data: {user, token}, statusCode: res.statusCode})
    } catch (err) {
  ...
  const token = utils.generateToken(user.id)
     ...
     */
      app.post(
          '/api/token', passport.authenticate('jwt', {session: false}),
          async (req, res, next) => {
            if (!req.user) return res.status(401).send('Invalid token')
              try {
                ...
                return res.status(400).send(
                    'Token manager temporarily desactivated.')
              }
          })
     ...
     */
    app.post(
        '/api/code/deezer', passport.authenticate('jwt', {session: false}),
        async (req, res, next) => {
        if (!req.user) return res.status(401).send('Invalid code')
            try {
            const ret = await axios.post(
                'https://connect.deezer.com/oauth/access_token.php?app_id=' +
                req.body.app_id + '&secret=' + req.body.secret +
                '&code=' + req.body.code + '&output=json')
            return res.status(200).json({
                status: 'success',
